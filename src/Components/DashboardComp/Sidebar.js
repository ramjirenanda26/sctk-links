...
const Sidebar = () => {
  ...
  return (
    <div className="sidebar p-2 pt-5">
      <div className="header-sidebar d-flex justify-content-center align-items-center" style={{ cursor: 'pointer' }}>
        <img src={logo} className="rounded logo-dashboard" alt="logo hopepoints" />
        <span className="brand-name fs-4">SCTK-Links</span>
      </div>
      ...
      <div className="list-group list-group-flush">
        ...
        <a href="/dashboard/registration" className="list-group-item py-3">
          <i className="fa-solid fa-table icon-sidebar"></i>
          <span className="text-sidebar">Registration</span>
        </a>
        {role === 'admin' && (
          <>
            <Nav.Link className="list-group-item py-3 d-flex align-items-center button-account" onClick={toggleDropdown}>
              <i className="fas fa-users icon-sidebar"></i>
              <span className="text-sidebar">Accounts</span>
              <i className={`fas fa-chevron-${isOpen ? 'up' : 'down'} ms-auto icon-dropdown`}></i>
            </Nav.Link>
